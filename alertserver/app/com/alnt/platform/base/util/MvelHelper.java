...
/**
 * Helper class for some common rule functions that can be called from mvel as
 * well as utility functions to make calling MVEL rules within Broadleaf easier.
 * 
 * An instance of this class is available to the mvel runtime under the variable
 * name MvelHelper with the following functions:
 * 
...
public class MvelHelper {
	...
	private static final Map<String, Serializable> DEFAULT_EXPRESSION_CACHE = new EfficientLRUMap<String, Serializable>(
			5000);
	...
	public static Object convertField(String type, String fieldValue) {
		...
		try {
			...
			} else if (type.equals("MONEY") || type.equals("DECIMAL")) {
	...
	/**
	 * Returns true if the passed in rule passes based on the passed in
	 * ruleParameters.
	 * 
	 * Also returns true if the rule is blank or null.
	 * 
	 * Calls the {@link #evaluateRule(String, Map, Map)} method passing in the
	 * DEFAULT_EXPRESSION_CACHE. For systems that need to cache a large number of
	 * rule expressions, an alternate cache can be passed in. The default cache is
	 * able to cache up to 1,000 rule expressions which should suffice for most
	 * systems.
	 * 
	 * @param rule
	 ...
	 * @param additionalContextImports
	 *            additional imports to give to the {@link ParserContext} besides
	 *            "MVEL" ({@link MVEL} and "MvelHelper" ({@link MvelHelper}) since
	...
	/**
	 * <p>
	 * Provides a hook point to modify the final expression before it's built. By
	 * default, this looks for attribute maps and replaces them such that it does
	 * string comparison.
	 * 
	 * <p>
	 * For example, given an expression like getProductAttributes()['somekey'] ==
	 * 'someval', getProductAttributes()['somekey'] actually returns a
	 * ProductAttribute object, not a String, so the comparison is wrong. Instead,
	 * we actually want to do this: getProductAttributes().?get('somekey').?value ==
	 * 'someval'. This function performs that replacement
	 *
	 * <p>
	...
	protected static String modifyExpression(String rule, Map<String, Object> ruleParameters, ParserContext context) {
		...
		for (String attributeMap : getRuleAttributeMaps()) {
			modifiedExpression = modifiedExpression.replaceAll(attributeMap + "\\(\\)\\[(.*?)\\](?!\\.\\?value)",
	...
	/**
	 * Returns an array of attribute map field names that we need to do replacements
	 * for in {@link #modifyExpression(String, Map, ParserContext)}
	 */
	protected static String[] getRuleAttributeMaps() {
		// intentionally left out pricing context getPricingContextAttributes because
		// that's a Map<String, String>
		return new String[] { "getProductAttributes", "getCategoryAttributesMap", "getSkuAttributes",
				"getOrderItemAttributes", "getCustomerAttributes",
	...
	/**
	 * When true, LOG.info statement will be suppressed. Should only be set from
	 * within MvelHelperTest. Prevents an error from displaying during unit test
	 * runs.
	 * 
	 * @param testMode
	 */
